...
static int init(int argc, char *argv[])
{
    ...
    while (i < argc)
    {
        ...
        if (!gotTwoPartOption)
        {
            ...
            {
                if (sscanf(&(argv[i][5]), "%f", &tempF) == 1 && tempF > 0.0f)
                    dpi = tempF;
                else
                    ARLOGe("Error: argument '%s' to -dpi= invalid.\n", argv[i]);
            }
            else
            {
                if (!inputFilePath)
                    inputFilePath = strdup(argv[i]);
                else
                    usage(argv[0]);
            }
        }
    ...
    if (strcmp(ext, "iset") == 0)
    {
        ...
        {
            if (imageSet->scale[i]->dpi > imageSet->scale[targetScale]->dpi)
                targetScale = i;
        }
    ...
    while (xsize > windowWidth * xzoom)
        xzoom += 1.0;

    while (ysize > windowHeight * yzoom)
        yzoom += 1.0;
...
static void   keyEvent(unsigned char key, int x, int y)
{
    ...
    case 'B':
        ...
        if (mode == AR_LABELING_BLACK_REGION)
            mode = AR_LABELING_WHITE_REGION;
        else
            mode = AR_LABELING_BLACK_REGION;
...
#define helpTextLineCount (sizeof(helpText) / sizeof(char*))
    ...
    for (i = 0; i < helpTextLineCount; i++)
        print(helpText[i], 2.0f, (helpTextLineCount - 1 - i) * 12.0f + 2.0f, 0, 0);
