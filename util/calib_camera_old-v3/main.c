...
static void     save_param(void)
{
    ...
    for (i = 0; i < AR_DIST_FACTOR_NUM_MAX; i++)
        param.dist_factor[i] = dist_factor[i];
...
static void init(int argc, char *argv[])
{
    ...
    if ((pixelFormat = arVideoGetPixelFormat()) < 0)
        exit(0);

    if ((pixelSize = arVideoGetPixelSize()) < 0)
        exit(0);
...
static void mouseEvent(int button, int state, int x, int y)
{
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_UP)
    {
        ...
        else if (status == 1)
        {
            ...
            if (point_num == patt.h_num * patt.v_num)
                print_comment(2);
        }
        else if (status == 2)
        {
