    ...
    if (viewContexts)
    {
        for (i = 0; i < viewContextCount; i++)
        {
            if (viewContexts[i].arglSettings)
                arglCleanup(viewContexts[i].arglSettings);
...
static void calibModeNext(void)
{
    ...
    if (calibMode == MODE_CALIBRATION_NOT_STARTED || calibMode == MODE_CALIBRATION_COMPLETED)
    {
        ...
        for (viewIndex = 0; views[viewIndex].viewEye != calibrationEye; viewIndex++)
            ;                                                                         // Locate the view we're calibrating.
        ...
        if (gPatt_found)
        {
            ...
            if (co2 == CALIB_POS2_NUM)
            {
                ...
                if (co1 == CALIB_POS1_NUM)
                {
                    ...
                    if (stereoDisplayMode == STEREO_DISPLAY_MODE_INACTIVE)
                        calibFilenameDefault = calibFilenameMono;
                    else
                    {
                        if (calibrationEye == VIEW_RIGHTEYE)
                            calibFilenameDefault = calibFilenameStereoR;
                        else
                            calibFilenameDefault = calibFilenameStereoL;
                    }
...
static void DisplayPerContext(const int contextIndex)
{
    ...
    if (gVideoSeeThrough)
        gARTImage = NULL;                   // All views done, can invalidate image data now.
}
