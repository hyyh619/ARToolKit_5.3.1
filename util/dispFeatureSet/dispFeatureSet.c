...
static int init(int argc, char *argv[])
{
    ...
    {
        if (imageSet->scale[i]->xsize > xmax)
            xmax = imageSet->scale[i]->xsize;

        if (imageSet->scale[i]->ysize > ymax)
            ymax = imageSet->scale[i]->ysize;
    }

    xzoom = yzoom = 1.0;

    while (xmax > winXsize * xzoom)
        xzoom += 1.0;

    while (ymax > winYsize * yzoom)
        yzoom += 1.0;
...
static void dispFunc(void)
{
    ...
    if (display_fset3)
    {
        ...
        {
            if (refDataSet->refPoint[i].refImageNo != page)
                continue;
