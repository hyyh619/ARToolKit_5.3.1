...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            if (sscanf(&argv[i][5], "%f", &dpi) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-sd_thresh=", 11) == 0)
        {
            if (sscanf(&argv[i][11], "%f", &sd_thresh) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-max_thresh=", 12) == 0)
        {
            if (sscanf(&argv[i][12], "%f", &max_thresh) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-min_thresh=", 12) == 0)
        {
            if (sscanf(&argv[i][12], "%f", &min_thresh) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-feature_density=", 13) == 0)
        {
            if (sscanf(&argv[i][13], "%d", &featureDensity) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-level=", 7) == 0)
        {
            if (sscanf(&argv[i][7], "%d", &tracking_extraction_level) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-leveli=", 8) == 0)
        {
            if (sscanf(&argv[i][8], "%d", &initialization_extraction_level) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-max_dpi=", 9) == 0)
        {
            if (sscanf(&argv[i][9], "%f", &dpiMax) != 1)
                usage(argv[0]);
        }
        else if (strncmp(argv[i], "-min_dpi=", 9) == 0)
        {
            if (sscanf(&argv[i][9], "%f", &dpiMin) != 1)
                usage(argv[0]);
        }
    ...
    if (logfile[0])
    {
        if (!freopen(logfile, "a", stdout) ||
            ...
            ARLOGe("Unable to redirect stdout or stderr to logfile.\n");
    }
        ...
        if (tracking_extraction_level == -1 && (sd_thresh == -1.0 || min_thresh == -1.0 || max_thresh == -1.0 || occ_size == -1))
        {
            do
            {
                ...
                if (buf[0] == '\n')
                    ...
                    sscanf(buf, "%d", &tracking_extraction_level);
            }
        ...
        if (initialization_extraction_level == -1 && featureDensity == -1)
        {
            do
            {
                ...
                if (buf[0] == '\n')
                    ...
                    sscanf(buf, "%d", &initialization_extraction_level);
            }
...
static int setDPI(void)
{
    ...
    if (dpiMin == -1.0f)
    {
        for (;;)
        {
            ...
            if (fgets(buf1, 256, stdin) == NULL)
                EXIT(E_USER_INPUT_CANCELLED);

            if (sscanf(buf1, "%f", &dpiMin) == 0)
                continue;

            if (dpiMin >= dpiMinAllowable && dpiMin <= (dpiMax == -1.0f ? dpi : dpiMax))
                break;
            else
                printf("Error: you entered %.3f, but value must be greater than or equal to %.3f and less than or equal to %.3f.\n", dpiMin, dpiMinAllowable, (dpiMax == -1.0f ? dpi : dpiMax));
        }
    }
    ...
    if (dpiMax == -1.0f)
    {
        for (;;)
        {
            ...
            if (sscanf(buf1, "%f", &dpiMax) == 0)
                continue;

            if (dpiMax >= dpiMin && dpiMax <= dpi)
                break;
            else
                printf("Error: you entered %.3f, but value must be greater than or equal to minimum resolution (%.3f) and less than or equal to image resolution (%.3f).\n", dpiMax, dpiMin, dpi);
        }
    }
    else if (dpiMax > dpi)
    {
