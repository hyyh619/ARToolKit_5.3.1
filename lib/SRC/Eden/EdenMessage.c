        ...
        do
        {
            ...
            else
            {
                ...
                if (addChar)
                {
                    ...
                    if (lineLength == BOX_LINE_LENGTH_MAX)
                        newline = true;                                    // Next char would overflow buffer, so break now.
            ...
            if (newline)
            {
                ...
                settings->lineCount++;
                if (settings->lineCount == BOX_LINES_MAX)
                    done = true;
...
static void boxDestroy(boxSettings_t **settings_p)
{
    ...
    {
        for (i = 0; i < (*settings_p)->lineCount; i++)
            free((*settings_p)->lines[i]);
...
EDEN_E_t EdenMessageInputShow(const unsigned char *prompt, const unsigned int minLength, const unsigned int maxLength, int intOnly, int fpOnly, int alphaOnly)
{
    ...
    if (maxLength == 0)
        gInputLengthMax = EDEN_MESSAGE_INPUT_MAX_LENGTH_DEFAULT;
    else
...
EDEN_BOOL EdenMessageInputKeyboard(const unsigned char keyAsciiCode)
{
    ...
    if (gInputPtr && !gInputComplete)
    {
        ...
        default:
            ...
            if (gInputIntOnly && (keyAsciiCode < '0' || keyAsciiCode > '9'))
                break;

            if (gInputFPOnly && (keyAsciiCode < '0' || keyAsciiCode > '9') && keyAsciiCode != '.')
                break;

            if (gInputAlphaOnly && (keyAsciiCode < 'A' || keyAsciiCode > 'Z') && (keyAsciiCode < 'a' || keyAsciiCode > 'z'))
                break;
