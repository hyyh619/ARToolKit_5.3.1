...
float EdenGLFontGetCharacterWidth(const unsigned char c)
{
    ...
    case EDEN_GL_FONT_TYPE_GLUT_STROKE:
        ...
        else
        {
            ...
            if (c == ' ' && gFormattingSettings.wordExtraSpacing)
                widthAtSizeOfOnePoint *= (gFormattingSettings.wordExtraSpacing + 1.0f);
...
EDEN_BOOL EdenGLFontLoadTextureFontForContext(const int contextIndex, EDEN_GL_FONT_INFO_t *fontInfo)
{
    ...
    // Sanity checks.
    if (contextIndex < 0 || contextIndex >= gContextsActiveCount)
        return (FALSE);
...
static void drawOneLine(const unsigned char *line)
{
    ...
    if (gFontSettings.font->type == EDEN_GL_FONT_TYPE_GLUT_STROKE)
    {
        while ((c = line[i++]))
        {
            ...
            if (!gFontSettings.font->monospaced && c == ' ' && gFormattingSettings.wordExtraSpacing)
                glTranslatef(glutStrokeWidth(gFontSettings.font->tsi, ' ') * gFormattingSettings.wordExtraSpacing, 0.0f, 0.0f);
...
void EdenGLFontDrawLine(const int contextIndex, const unsigned char *line, const float hOffset, const float vOffset, H_OFFSET_TYPE hOffsetType, V_OFFSET_TYPE vOffsetType)
{
    ...
    if (hOffsetType == H_OFFSET_VIEW_LEFT_EDGE_TO_TEXT_LEFT_EDGE)
        x = hOffset;
    else
    {
        float textWidth = EdenGLFontGetLineWidth(line);
        if (hOffsetType == H_OFFSET_TEXT_RIGHT_EDGE_TO_VIEW_RIGHT_EDGE)
            x = gViewSettings.width - hOffset - textWidth;
        else /* H_OFFSET_VIEW_CENTER_TO_TEXT_CENTER */
            x = (gViewSettings.width - textWidth) / 2.0f + hOffset;
    }

    if (vOffsetType == V_OFFSET_VIEW_BOTTOM_TO_TEXT_BASELINE)
        y = vOffset;
    else
    {
        float textHeight = EdenGLFontGetHeight();
        if (vOffsetType == V_OFFSET_VIEW_TEXT_TOP_TO_VIEW_TOP)
            y = gViewSettings.height - hOffset - textHeight;
        else /* V_OFFSET_VIEW_CENTER_TO_TEXT_CENTER */
            y = (gViewSettings.height - textHeight) / 2.0f + hOffset;
    }

    if (gFontSettings.font->type == EDEN_GL_FONT_TYPE_TEXTURE)
        drawSetup(contextIndex, &VTs);
