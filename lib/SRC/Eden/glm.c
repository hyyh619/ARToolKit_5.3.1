...
static GLMgroup* glmFindGroup(GLMmodel *model, char *name, const GLuint material)
{
    ...
    while (group)
    {
        if (!strcmp(name, group->name) && group->material == material)
            break;                                                            // Both name and material must match.
...
glmFirstPass(GLMmodel *model, FILE *file, const int contextIndex, const GLboolean readTexturesNow)
{
    ...
    while (fscanf(file, "%s", buf) != EOF)
    {
        ...
        case 'm':                               /* material library */
            ...
            if (model->mtllibname)
            {
                if (strcmp(buf, model->mtllibname) != 0)
                    EDEN_LOGe("glmFirstPass(): Warning: Multiple material library names found. Only the first will be used.\n");
            }
            else
            {
                if (glmReadMTL(model, buf, contextIndex, readTexturesNow))
...
glmDraw(GLMmodel *model, GLuint mode, const int contextIndex)
{
    ...
    for (transparencyPass = 0; transparencyPass <= 1; transparencyPass++)
    {
        ...
        for (group = model->groups; group; group = group->next)
        {
            ...
            if ((!transparencyPass && hasTransparency) || (transparencyPass && !hasTransparency))
                continue;                                                                                   // Skip if group is wrong sort. (XOR).
...
GLvoid glmCreateArrays(GLMmodel *model, GLuint mode)
{
    ...
    while (group)
    {
        if (group->numtriangles)
        {
            ...
            if (needNormals || needFacetnorms)
                normals = (GLfloat*)malloc(sizeof(GLfloat) * 3 * group->numtriangles * 3);                                 // 3 floats per normal, 3 normals per triangle.
            else
                normals = NULL;

            if (needTexcoords)
                texcoords = (GLfloat*)malloc(sizeof(GLfloat) * 2 * group->numtriangles * 3);                 // 2 floats per texcoord, 3 texcoords per triangle.
            else
                texcoords = NULL;
            ...
            for (i = 0; i < 3 * vntCount; i++)
                array->vertices[i] = vertices[i];
            ...
            if (needTexcoords)
            {
                ...
                for (i = 0; i < 2 * vntCount; i++)
                    array->texcoords[i] = texcoords[i];
            ...
            if (group->material)
                array->material = &(model->materials[group->material]);
        } // if group->numtriangles
...
GLvoid glmDeleteArrays(GLMmodel *model)
{
    ...
    while (arrayHead)
    {
        ...
        if (arrayHead->normals)
            free(arrayHead->normals);

        if (arrayHead->texcoords)
            free(arrayHead->texcoords);
...
GLvoid glmDrawArrays(GLMmodel *model, const int contextIndex)
{
    ...
    for (transparencyPass = 0; transparencyPass <= 1; transparencyPass++)
    {
...

        {
            ...
            if ((!transparencyPass && hasTransparency) || (transparencyPass && !hasTransparency))
                continue;                                                                                   // Skip if array is wrong sort. (XOR).
