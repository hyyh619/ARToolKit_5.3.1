...
#include <Eden/EdenMath.h>      // MIN(), MAX()
...
{
    if (gSurfacesContextsActiveCount)
        return (FALSE);
...
EDEN_BOOL EdenSurfacesTextureLoad2(const int contextIndex, const int numTextures, const TEXTURE_INFO_t *textureInfo, TEXTURE_INDEX_t *textureIndices, char *hasAlpha_out, const EDEN_BOOL flipH, const EDEN_BOOL flipV)
{
...
#endif
    if (hasAlpha_out)
        *hasAlpha_out = 0;               // Will be set if any textures have alpha.
    ...
    for (i = 0; i < numTextures; i++)
    {
        ...
        {
            ptr++;
            if (ptr == gTextureIndexMax)
                ptr = 0;                                      // Loop around.

            if (ptr == gTextureIndexPtr[contextIndex])
                break;                                                    // Quit if we're back where we started.
        }
        ...
        else
        {
            ...
            if (flipH || flipV)
            {
                ...
                p0 = p1 = data;
                if (flipV)
                    p1 += (sizeY - 1) * sizeX * nc;    // Move to last row.

                if (flipH)
                    p1 += (sizeX - 1) * nc;      // Move to last column.
            ...
            {
                if (textureInfo[i].internalformat == GL_ALPHA)
                    format = GL_ALPHA;
                else
                    format = GL_LUMINANCE;
            }
            ...
            else
            {
                ...
                while (textureSizeX < sizeX)
                    textureSizeX <<= 1;

                while (textureSizeY < sizeY)
                    textureSizeY <<= 1;
            }
            ...
            if (hasAlpha_out)
                *hasAlpha_out = (format == GL_RGBA || format == GL_ALPHA);
        }
    }
...
EDEN_BOOL EdenSurfacesDraw(const int contextIndex, const TEXTURE_INDEX_t textureIndex, const int width, const int height, const EDEN_TEXTURE_SCALING_MODE scaleMode, const EDEN_TEXTURE_ALIGNMENT_MODE alignMode)
{
    ...
    if (textureIndex < 1 || textureIndex > gTextureIndexMax)
        return (FALSE);
    ...
    if (alignMode == TOP_LEFT || alignMode == LEFT || alignMode == BOTTOM_LEFT)
    {
        S0 = 0.0f; S1 = S;
    }
    else if (alignMode == TOP_RIGHT || alignMode == RIGHT || alignMode == BOTTOM_RIGHT)
    {
        S0 = maxS - S; S1 = maxS;
    }
    else
    {
        S0 = maxS * 0.5f - S * 0.5f; S1 = maxS * 0.5f + S * 0.5f;
    }

    if (alignMode == BOTTOM_LEFT || alignMode == BOTTOM || alignMode == BOTTOM_RIGHT)
    {
        T0 = 0.0f; T1 = T;
    }
    else if (alignMode == TOP_LEFT || alignMode == TOP || alignMode == TOP_RIGHT)
    {
        T0 = maxT - T; T1 = maxT;
    }
    else
    {
        T0 = maxT * 0.5f - T * 0.5f; T1 = maxT * 0.5f + T * 0.5f;
    }
...
int EdenGLCapabilityCheck(const unsigned short minVersion, const unsigned char *extension)
{
    ...
    if (extension)
    {
        ...
        if (EdenGluCheckExtension(extension, strExtensions))
            return (TRUE);
    }
