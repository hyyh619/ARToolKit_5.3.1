...
AR2VideoParamQuickTime7T* ar2VideoOpenQuickTime7(const char *config)
{
            ...
            else if (strncmp(a, "-format=", 8) == 0)
            {
                ...
                else
                {
                    ...
                    if (strlen(line) == 12)
                        ...
                        err_i = (sscanf(&line[8], "%li", (long*)&pixFormat) < 1);                       // Integer.
                }
            }
    ...
    @autoreleasepool {
        ...
        if (source)
            vid->qtKitVideo.preferredDevice = source;
...
int ar2VideoCloseQuickTime7(AR2VideoParamQuickTime7T *vid)
{
    if (vid)
    {
        @autoreleasepool {
            ...
            if (vid->qtKitVideoGotFrameDelegate)
                [vid->qtKitVideoGotFrameDelegate release];
        }
        free(vid);
        return 0;
    }
        ...
        if (vid->qtKitVideo)
        {
            @autoreleasepool {
                ...
                if (frame)
                {
                    ...
                    if (!CVPixelBufferGetPlaneCount(frame))
                        vid->buffer.buff = CVPixelBufferGetBaseAddress(frame);
                    else
...
- (void) QTKitVideoGotFrame:(id)sender userData:(void*)data
{
    ...
    {
        if (vid->gotImageFunc)
            (*vid->gotImageFunc)(ar2VideoGetImageQuickTime7(vid), vid->gotImageFuncUserData);
    }
}
            ...
            if (gotImageFunc != vid->gotImageFunc || userData != vid->gotImageFuncUserData)
            {
                @autoreleasepool {
                    ...
                    if (gotImageFunc)
                    {
                        ...
                        if (!vid->qtKitVideoGotFrameDelegate)
                            vid->qtKitVideoGotFrameDelegate = [[videoQuickTime7QTKitVideoGotFrameDelegate alloc] init];                                                           // Create an instance of delegate.
