...
unsigned char* videoAndroidNativeCaptureGetFrame(VIDEO_ANDROID_NATIVE_CAPTURE *nc)
{
    ...
    if (nc->frameBuffersStatus[0] == LOCKED)
        nc->frameBuffersStatus[0] = EMPTY;

    if (nc->frameBuffersStatus[1] == LOCKED)
        nc->frameBuffersStatus[1] = EMPTY;

    if (nc->frameBuffersStatus[0] == READY || nc->frameBuffersStatus[0] == NOTIFIED)
        frameReadyIndex = 0;
    else if (nc->frameBuffersStatus[1] == READY || nc->frameBuffersStatus[1] == NOTIFIED)
        frameReadyIndex = 1;
    else
    {
        pthread_mutex_unlock(&nc->frameLock);
