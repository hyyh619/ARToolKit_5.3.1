...
AR2VideoParamAndroidT* ar2VideoOpenAndroid(const char *config)
{
    ...
    if (a != NULL)
    {
        for (;;)
        {
            ...
            {
                if (sscanf(&line[8], "%d", &vid->camera_index) == 0)
                    err_i = 1;
#endif
            }
            else
    ...
    // Initial state.
    if (!vid->format)
        vid->format = AR_INPUT_ANDROID_PIXEL_FORMAT;

    if (!vid->focal_length)
        vid->focal_length = AR_VIDEO_ANDROID_FOCAL_LENGTH_DEFAULT;
...
static void cparamSeachCallback(CPARAM_SEARCH_STATE state, float progress, const ARParam *cparam, void *userdata)
{
    ...
    case CPARAM_SEARCH_STATE_RESULT_NULL:
        if (vid->cparamSearchCallback)
            (*vid->cparamSearchCallback)(NULL, vid->cparamSearchUserdata);
    ...
    case CPARAM_SEARCH_STATE_OK:
        if (vid->cparamSearchCallback)
            (*vid->cparamSearchCallback)(cparam, vid->cparamSearchUserdata);
