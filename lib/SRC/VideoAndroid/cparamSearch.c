...
{
    if (!ptr || !size || !nmemb)
        return (0);                          // If no data, do nothing.
    ...
    while (threadStartWait(threadHandle) == 0)
    {
        ...
        while (searchData)
        {
            ...
            if (searchData->callback)
                (*searchData->callback)(result, 0.0f, NULL, searchData->userdata);
            ...
            if (result == CPARAM_SEARCH_STATE_IN_PROGRESS)
            {
                ...
                if (result == CPARAM_SEARCH_STATE_IN_PROGRESS)
                {
                    ...
                    for (i = 0; i < SEARCH_TOKEN_LENGTH; i++)
                        snprintf(&(searchToken_ascii[i * 2]), 3, "%.2hhx", searchToken[i]);
            ...
            if (searchData->device_id)
                free(searchData->device_id);

            if (searchData->aspect_ratio)
                free(searchData->aspect_ratio);

            if (searchData->camera_para_base64)
                free(searchData->camera_para_base64);
