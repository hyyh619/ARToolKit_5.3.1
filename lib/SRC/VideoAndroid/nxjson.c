...
#  define NX_JSON_REPORT_ERROR(msg, p) __android_log_print(ANDROID_LOG_ERROR, "nxjson", "NXJSON PARSE ERROR (%d): " msg " at %s\n", __LINE__, p)
#endif
...
static int unicode_to_utf8(unsigned int codepoint, char *p, char **endp)
{
    ...
    if (codepoint < 0x80)
        ...
        *p++ = 240 + codepoint / 262144, *p++ = 128 + codepoint / 4096 % 64, *p++ = 128 + codepoint / 64 % 64, *p++ = 128 + codepoint % 64;
    else
        return 0;  // error
