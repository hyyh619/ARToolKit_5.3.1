...
static char arglSetupProgram(ARGL_CONTEXT_SETTINGS_REF contextSettings)
{
    ...
    if (contextSettings->program)
        arglGLDestroyShaders(0, 0, contextSettings->program);
    ...
    if (contextSettings->format == AR_PIXEL_FORMAT_420f)
        fragShaderString = fragShaderStringYCbCrITURec601FullRangeBiPlanar;
    else if (contextSettings->format == AR_PIXEL_FORMAT_420v)
        fragShaderString = fragShaderStringYCbCrITURec601VideoRangeBiPlanar;
    else if (contextSettings->format == AR_PIXEL_FORMAT_NV21)
        fragShaderString = fragShaderStringYCrCbITURec601FullRangeBiPlanar;
    else
        fragShaderString = fragShaderStringRGB;
...
char arglPixelFormatSet(ARGL_CONTEXT_SETTINGS_REF contextSettings, AR_PIXEL_FORMAT format)
{
    ...
    if (!arglSetupProgram(contextSettings))
        return (FALSE);
