...
char arglPixelBufferSizeSet(ARGL_CONTEXT_SETTINGS_REF contextSettings, int bufWidth, int bufHeight)
{
    ...
    else
    {
        ...
        while (contextSettings->textureSizeX < bufWidth)
            contextSettings->textureSizeX <<= 1;

        while (contextSettings->textureSizeY < bufHeight)
...
char arglPixelBufferDataUploadBiPlanar(ARGL_CONTEXT_SETTINGS_REF contextSettings, ARUint8 *bufDataPtr0, ARUint8 *bufDataPtr1)
{
    ...
    if (!contextSettings->textureObjectsHaveBeenSetup || !contextSettings->textureGeometryHasBeenSetup || !contextSettings->pixSize)
        return (FALSE);
    ...
    if (contextSettings->bufSizeIsTextureSize)
    {
        ...
        if (bufDataPtr1 && contextSettings->useTextureYCbCrBiPlanar)
        {
            ...
            {
                if (contextSettings->format == AR_PIXEL_FORMAT_NV21)
                    arglCrCbToRGBA_ARM_neon_asm(contextSettings->bufRGBAdd, contextSettings->bufRGBSubtract, bufDataPtr1, sizeX * sizeY);
                else
                    arglCbCrToRGBA_ARM_neon_asm(contextSettings->bufRGBAdd, contextSettings->bufRGBSubtract, bufDataPtr1, sizeX * sizeY);
            }
            else
            {
#endif
            if (contextSettings->format == AR_PIXEL_FORMAT_NV21)
                arglCrCbToRGBA(contextSettings->bufRGBAdd, contextSettings->bufRGBSubtract, bufDataPtr1, sizeX * sizeY);
            else
                arglCbCrToRGBA(contextSettings->bufRGBAdd, contextSettings->bufRGBSubtract, bufDataPtr1, sizeX * sizeY);
...
int arglGLCapabilityCheck(const unsigned short minVersion, const unsigned char *extension)
{
    ...
    if (extension)
    {
        ...
        if (arglGluCheckExtension(extension, strExtensions))
            return (TRUE);
    }
