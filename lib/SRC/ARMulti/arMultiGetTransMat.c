                                           ...
                                           ARMultiMarkerInfoT *config, int robustFlag)
{
    ...
    for (i = 0; i < config->marker_num; i++)
    {
        ...
        if (config->marker[i].patt_type == AR_MULTI_PATTERN_TYPE_TEMPLATE)
        {
            ...
            {
                if (marker_info[j].idPatt != config->marker[i].patt_id)
                    continue;

                if (marker_info[j].cfPatt < config->cfPattCutoff)
                    continue;

                if (k == -1)
                    k = j;
                else if (marker_info[k].cfPatt < marker_info[j].cfPatt)
                    k = j;
            }

            config->marker[i].visible = k;
            if (k >= 0)
                marker_info[k].dir = marker_info[k].dirPatt;
        }
        else   // config->marker[i].patt_type == AR_MULTI_PATTERN_TYPE_MATRIX
        {
            for (j = 0; j < marker_num; j++)
            {
                ...
                {
                    if (marker_info[j].globalID != config->marker[i].globalID)
                        continue;
                }
                else
                {
                    if (marker_info[j].idMatrix != config->marker[i].patt_id)
                        continue;
                }

                if (marker_info[j].cfMatrix < config->cfMatrixCutoff)
                    continue;

                if (k == -1)
                    k = j;
                else if (marker_info[k].cfMatrix < marker_info[j].cfMatrix)
                    k = j;
            }

            config->marker[i].visible = k;
            if (k >= 0)
                marker_info[k].dir = marker_info[k].dirMatrix;
        }
    ...
    for (i = 0; i < config->marker_num; i++)
    {
        if ((j = config->marker[i].visible) < 0)
            continue;
        ...
        if (err > AR_MULTI_POSE_ERROR_CUTOFF_EACH_DEFAULT)
        {
            ...
            if (marker_info[j].cutoffPhase == AR_MARKER_INFO_CUTOFF_PHASE_NONE)
                marker_info[j].cutoffPhase = AR_MARKER_INFO_CUTOFF_PHASE_POSE_ERROR;
    ...
    for (i = 0; i < config->marker_num; i++)
    {
        if ((k = config->marker[i].visible) < 0)
            continue;
    ...
    else
    {
        ...
        for (i = 0; i < config->marker_num; i++)
        {
            ...
            if (marker_info[k].cutoffPhase == AR_MARKER_INFO_CUTOFF_PHASE_NONE)
                marker_info[k].cutoffPhase = AR_MARKER_INFO_CUTOFF_PHASE_POSE_ERROR_MULTI;
        }
    }
