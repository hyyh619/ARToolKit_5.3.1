...
ARMultiMarkerInfoT* arMultiReadConfigFile(const char *filename, ARPattHandle *pattHandle)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            if ((marker[i].globalID & 0xffff8000ULL) == 0ULL)
                marker[i].patt_id = (int)(marker[i].globalID & 0x00007fffULL);                                               // If upper 33 bits are zero, use lower 31 bits as regular matrix code.
            else
                marker[i].patt_id = 0;
