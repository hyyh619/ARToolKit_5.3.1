...
int arOSGLoadModel2(AROSG *arOsg, const char *modelFilePath, const ARdouble translation[3], const ARdouble rotation[4], const ARdouble scale[3])
{
    if (!arOsg)
        return (-1);
    ...
    while (imageExtensions[i] != ext && imageExtensions[i] != "")
        i++;
...
#ifndef DEBUG_AROSG_MODELLOADING
    if (translation)
        configTransform->preMult(osg::Matrix::translate(translation[0], translation[1], translation[2]));

    if (rotation)
        configTransform->preMult(osg::Matrix::rotate(osg::inDegrees(rotation[0]), rotation[1], rotation[2], rotation[3]));
...
int arOSGLoadModel(AROSG *arOsg, const char *modelDescriptionFilePath)
{
    ...
    int lightingFlag = 1, transparencyFlag = -1;
    ...
    int index = arOSGLoadModel2(arOsg, modelFilePath, translation, rotation, scale);

    if (index >= 0)
    {
        if (!lightingFlag)
            arOSGSetModelLighting(arOsg, index, 0);

        if (transparencyFlag != -1)
            arOSGSetModelTransparency(arOsg, index, transparencyFlag);
    }
...
int arOSGUnloadModel(AROSG *arOsg, const int index)
{
    ...
    if (index < 0 || index >= AR_OSG_MODELS_MAX)
        return (-1);
...
int arOSGSetModelAnimationLoopModeOverride(AROSG *arOsg, const int index, const int mode)
{
    ...
    arOsg->models[index]->accept(manv);
    if (mode == 0)
        manv.setLoopMode(osg::AnimationPath::NO_LOOPING);
    else if (mode == 1)
        manv.setLoopMode(osg::AnimationPath::LOOP);
    else if (mode == 2)
        manv.setLoopMode(osg::AnimationPath::SWING);
...
void arOSGHandleMouseDownUp(AROSG *arOsg, const int button, const int state, const int x, const int y)
{
    ...
    {
        if (state == 0)
            arOsg->window->getEventQueue()->mouseButtonPress(x, y, button + 1);
        else
            arOsg->window->getEventQueue()->mouseButtonRelease(x, y, button + 1);
    }
}
