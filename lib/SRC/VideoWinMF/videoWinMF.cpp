...
#  error error ARToolKit Windows Media Foundation requires Windows 7 or Windows Server 2008 R2 or later. Please compile with Windows Windows 7 or Windows Server 2008 R2 SDK installed and with _WIN32_WINNT=0x0601 or later in your project compiler settings (setting /D_WIN32_WINNT=0x0601).
#endif
...
int ar2VideoDispOptionWinMF(void)
{
    ARLOG(" -device=WinMF\n");
    ARLOG(" -devNum=N\n");
    ARLOG("    Use device number N (default N=1).\n");
    ARLOG("    e.g. for a second camera input, N=2.\n");
    ARLOG(" -format=X\n");
    ARLOG("    Return images with pixels in format X, where X is one of: \n");
    ARLOG("    BGRA, BGR, NV12/420f, 2vuy/UYVY, yuvs/YUY2, RGB_565, RGBA_5551.\n");
    ARLOG(" -width=N\n");
    ARLOG("    Request video format of width N pixels.\n");
    ARLOG(" -height=N\n");
    ARLOG("    Request video format of height N pixels.\n");
    ARLOG(" -flipV\n");
    ARLOG("    Flip the image vertically.\n");
    ARLOG(" -noFlipV\n");
    ARLOG("    Do not flip the image vertically.\n");
    // ARLOG(" -flipH\n");
    // ARLOG("    Flip the image horizontally.\n");
    // ARLOG(" -noFlipH\n");
    // ARLOG("    Do not flip the image horizontally.\n");
    ARLOG(" -showDeviceList\n");
    ARLOG(" -noShowDeviceList\n");
    ARLOG(" -showFormats\n");
    ARLOG("    Dump the full list of native formats to the console.\n");
    ARLOG("    This is helpful in determining device capabilities.\n");
    ARLOG(" -noShowFormats\n");
    ARLOG("\n");
...
AR2VideoParamWinMFT* ar2VideoOpenWinMF(const char *config)
{
    ...
    // ARLOG("Entering ar2VideoOpenWinMF\n");
    if (ar2VideoWinMFRefCount == 0)
    {
        if (!ar2VideoWinMFInit2())
            return NULL;
    ...
    if (FAILED(hr))
    {
        if (hr == MF_E_DRM_UNSUPPORTED)
            ARLOGe("Error: The source contains protected content.\n");
        else
            ARLOGe("Error: MFCreateSourceReaderFromMediaSource (%#x).\n", hr);          // See WinError.h. E_INVALIDARG=0x80070057.
    ...
    {
        // Current settings are not handleable by ARToolKit.
        changeFormat = true;
    }
    else
    {
        ...
        {
            // Current settings are handleable by ARToolKit, but aren't what was requested.
            changeFormat = true;
        }
    }
    ...
    if (changeFormat)
    {
        ...
        for (dwMediaTypeIndex = 0;; dwMediaTypeIndex++)
        {
            ...
            if (currentPF != AR_PIXEL_FORMAT_INVALID && (vid->format == AR_PIXEL_FORMAT_INVALID || vid->format == currentPF))
                pfMatch = true;
            else
                pfMatch = false;

            if (currentWidth >= 0 && currentHeight >= 0 && (vid->width == -1 || vid->width == currentWidth) && (vid->height == -1 || vid->height == currentHeight))
                sizeMatch = true;
            else
                sizeMatch = false;

            if (pfMatch && dwPFMatchIndex == -1)
                dwPFMatchIndex = dwMediaTypeIndex;

            if (sizeMatch && dwSizeMatchIndex == -1)
                dwSizeMatchIndex = dwMediaTypeIndex;

            if (pfMatch && sizeMatch && dwPFAndSizeMatchIndex == -1)
                dwPFAndSizeMatchIndex = dwMediaTypeIndex;
        ...
        if (setWithNativeFormat)
        {
            ...
            if (FAILED(hr))
            {
                if (hr == MF_E_INVALIDMEDIATYPE)
                    ARLOGe("Error setting media type: At least one decoder was found for the native stream type, but the requested type specified was rejected.\n");
                else if (hr == MF_E_TOPO_CODEC_NOT_FOUND)
                    ARLOGe("Error setting media type: Could not find a decoder for the native stream type.\n");
                else
                    ARLOGe("Error: SetCurrentMediaType (%d).\n", hr);
...
AR2VideoBufferT* ar2VideoGetImageWinMF(AR2VideoParamWinMFT *vid)
{
    ...
    if (vid->format == AR_PIXEL_FORMAT_INVALID || vid->width <= 0 || vid->height <= 0)
        return NULL;
...
static bool     ar2VideoWinMFInit2(void)
{
    ...
    // ARLOG("Entering ar2VideoWinMFInit2\n");
...
static AR_PIXEL_FORMAT getARPixelFormatForMFMediaType(IMFMediaType *pType)
{
    ...
    if (subType == MFVideoFormat_RGB32 ||
        ...
        return AR_PIXEL_FORMAT_RGB_565;
    else if (subType == MFVideoFormat_RGB555)
        return AR_PIXEL_FORMAT_RGBA_5551;
...
static bool getMediaTypeInfo(IMFMediaType *pType, AR_PIXEL_FORMAT *pf, int *width, int *height, int *rowBytes)
{
    ...
    if (pf)
        *pf = getARPixelFormatForMFMediaType(pType);
        ...
        if (rowBytes)
        {
            ...
            if (FAILED(hr))
            {
                ...
                if (subType == MFVideoFormat_NV12)
                    ...
                    lStride = ((w * 3) + 3) & ~3;
                else if (subType == MFVideoFormat_RGB565 || subType == MFVideoFormat_RGB555)
                    lStride = ((w * 2) + 3) & ~3;
                else
                {
...
#endif // AR_INPUT_WINDOWS_MEDIA_FOUNDATION