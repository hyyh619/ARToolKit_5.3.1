...
static int jpegRead(FILE *fp, unsigned char *buf, int bufWidth, int bufHeight, AR_PIXEL_FORMAT bufPixFormat)
{
    ...
    if (convertPixFormat || cinfo.output_width > bufWidth)
    {
        ...
        for (i = 0; i < decompressBufRows; i++)
            decompressBufRowPtrs[i] = &(decompressBuf[decompressBufBytesPerRow * i]);
        ...
        while (cinfo.output_scanline < height)
        {
            ...
            if (!convertPixFormat)
            {
                ...
                for (i = 0; i < rowsread; i++)
                    memcpy(&(buf[bufBytesPerRow * (row + i)]), decompressBufRowPtrs[i], bufBytesPerRow);
            }
            else
            {
    ...
    else
    {
        ...
        while (cinfo.output_scanline < height)
        {
            ...
            for (i = 0; i < rowsToRead; i++)
                decompressBufRowPtrs[i] = &(buf[bufBytesPerRow * (row + i)]);
...
int ar2VideoDispOptionImage(void)
{
    ...
    ARLOG(" -image=pathnameOrURL");
    ARLOG(" -image=\"pathname Or URL\"");
    ARLOG("    specifies image to be read from file (or network, if supported).\n");
    ARLOG("    Multiple images may be specified by repeated use of this token.\n");
    ARLOG(" -loop\n");
    ARLOG("    After reading last image, next read will return first image.\n");
    ARLOG(" -noloop\n");
    ARLOG("    After reading last image, no further images will be returned.\n");
    ARLOG("\n");
...
AR2VideoParamImageT* ar2VideoOpenImage(const char *config)
{
        ...
        for (;;)
        {
            ...
            else if (strncmp(a, "-image=", 7) == 0)
            {
                ...
                else
                {
                    ...
                    if (!vid->imageList)
                        vid->imageList = imageRef;
                    else
                        imageListTail->next = imageRef;
            ...
            else if (strcmp(line, "-device=Image") == 0)
            {}
            else
            {
                err_i = 1;
    ...
    if (bufferpow2)
    {
        ...
        while (bufSizeX < vid->width)
            bufSizeX *= 2;

        while (bufSizeY < vid->height)
            bufSizeY *= 2;
    }
    else
    {
...
int ar2VideoCloseImage(AR2VideoParamImageT *vid)
{
    ...
    while (vid->imageList)
    {
        ...
        if (imageRefToFree->pathname)
            free (imageRefToFree->pathname);
...
AR2VideoBufferT* ar2VideoGetImageImage(AR2VideoParamImageT *vid)
{
    ...
    if (vid->nextImage)
    {
        ...
        if (!vid->nextImage && vid->loop)
            vid->nextImage = vid->imageList;                               // If we've hit the end of the list and looping requested, go back to head of linked list.
