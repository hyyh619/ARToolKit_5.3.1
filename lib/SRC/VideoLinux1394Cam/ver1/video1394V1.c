...
{
    if (portNum < 0 || portNum >= AR2VIDEO_1394_PORT_MAX)
        return NULL;
...
AR2VideoParam1394T* ar2VideoOpen1394(const char *config)
{
    ...
    if (initFlag == 0)
    {
        for (i = 0; i < AR2VIDEO_1394_PORT_MAX; i++)
            arV1394[i].isOpen = 0;
    ...
    {
        if (ar2VideoInit1394(vid->port, vid->debug) < 0)
            return NULL;
    ...
    for (j = 0; j < arV1394[vid->port].numCameras; j++)
    {
        ...
        if (vid->euid[0] != 0 || vid->euid[1] != 0)
        {
            ...
            if (vid->euid[0] != euid[0] || vid->euid[1] != euid[1])
                continue;
    ...
    {
        if (vid->debug)
            ARLOGe("cound not find the specified camera.\n");
...
static int ar2VideGetCameraEUID1394(int port, int node, quadlet_t euid[2])
{
    ...
    if (dc1394_get_camera_info(arV1394[port].handle, node, &caminfo) != DC1394_SUCCESS)
        return -1;
