...
EXPORT_API bool arwChangeToResourcesDir(const char *resourcesDirectoryPath)
{
    ...
    if (resourcesDirectoryPath)
        ok = (arUtilChangeToResourcesDirectory(AR_UTIL_RESOURCES_DIRECTORY_BEHAVIOR_USE_SUPPLIED_PATH, resourcesDirectoryPath, NULL) == 0);
    else
...
#else
    if (resourcesDirectoryPath)
        ok = (arUtilChangeToResourcesDirectory(AR_UTIL_RESOURCES_DIRECTORY_BEHAVIOR_USE_SUPPLIED_PATH, resourcesDirectoryPath) == 0);
    else
        ok = (arUtilChangeToResourcesDirectory(AR_UTIL_RESOURCES_DIRECTORY_BEHAVIOR_BEST, NULL) == 0);
#endif
#ifdef DEBUG
    char buf[MAXPATHLEN];
...
EXPORT_API bool arwGetVideoParams(int *width, int *height, int *pixelSize, char *pixelFormatStringBuffer, int pixelFormatStringBufferLen)
{
    ...
    if (!gARTK->videoParameters(0, width, height, &pf))
        return false;

    if (pixelSize)
        *pixelSize = arUtilGetPixelSize(pf);
...
EXPORT_API bool arwGetVideoParamsStereo(int *widthL, int *heightL, int *pixelSizeL, char *pixelFormatStringBufferL, int pixelFormatStringBufferLenL, int *widthR, int *heightR, int *pixelSizeR, char *pixelFormatStringBufferR, int pixelFormatStringBufferLenR)
{
    ...
    if (!gARTK->videoParameters(0, widthL, heightL, &pfL))
        return false;

    if (!gARTK->videoParameters(1, widthR, heightR, &pfR))
        return false;

    if (pixelSizeL)
        *pixelSizeL = arUtilGetPixelSize(pfL);

    if (pixelSizeR)
        *pixelSizeR = arUtilGetPixelSize(pfR);
...
EXPORT_API void UnityRenderEvent(int eventID)
{
    ...
    case ARW_UNITY_RENDER_EVENTID_UPDATE_TEXTURE_GL:
        if (arwUnityRenderEventUpdateTextureGLTextureID_L)
            arwUpdateTextureGL(arwUnityRenderEventUpdateTextureGLTextureID_L);

        break;

    case ARW_UNITY_RENDER_EVENTID_UPDATE_TEXTURE_GL_STEREO:
        if (arwUnityRenderEventUpdateTextureGLTextureID_L && arwUnityRenderEventUpdateTextureGLTextureID_R)
            arwUpdateTextureGLStereo(arwUnityRenderEventUpdateTextureGLTextureID_L, arwUnityRenderEventUpdateTextureGLTextureID_R);
        break;
#endif  // !TARGET_PLATFORM_WINRT
...
EXPORT_API bool arwQueryMarkerVisibility(int markerUID)
{
    ...
    {
        gARTK->logv("arwQueryMarkerVisibility(): Couldn't locate marker with UID %d.", markerUID);
        return false;
    }
...
EXPORT_API bool arwQueryMarkerTransformation(int markerUID, float matrix[16])
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwQueryMarkerTransformation(): Couldn't locate marker with UID %d.", markerUID);
        return false;
    }

    for (int i = 0; i < 16; i++)
        matrix[i] = (float)marker->transformationMatrix[i];
...
EXPORT_API bool arwQueryMarkerTransformationStereo(int markerUID, float matrixL[16], float matrixR[16])
{
    ...
    for (int i = 0; i < 16; i++)
        matrixL[i] = (float)marker->transformationMatrix[i];

    for (int i = 0; i < 16; i++)
        matrixR[i] = (float)marker->transformationMatrixR[i];
...
EXPORT_API int arwGetMarkerPatternCount(int markerUID)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwGetMarkerPatternCount(): Couldn't locate marker with UID %d.", markerUID);
        return 0;
    }
...
EXPORT_API bool arwGetMarkerPatternConfig(int markerUID, int patternID, float matrix[16], float *width, float *height, int *imageSizeX, int *imageSizeY)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwGetMarkerPatternConfig(): Couldn't locate marker with UID %d.", markerUID);
        return false;
    }

    if (!(p = marker->getPattern(patternID)))
    {
        gARTK->logv("arwGetMarkerPatternConfig(): Marker with UID %d has no pattern with ID %d.", markerUID, patternID);
        return false;
    }
    ...
    {
        for (int i = 0; i < 16; i++)
            matrix[i] = (float)p->m_matrix[i];
    }

    if (width)
        *width = (float)p->m_width;

    if (height)
        *height = (float)p->m_height;

    if (imageSizeX)
        *imageSizeX = p->m_imageSizeX;

    if (imageSizeY)
        *imageSizeY = p->m_imageSizeY;
...
EXPORT_API bool arwGetMarkerPatternImage(int markerUID, int patternID, Color *buffer)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwGetMarkerPatternImage(): Couldn't locate marker with UID %d.", markerUID);
        return false;
    }

    if (!(p = marker->getPattern(patternID)))
    {
        gARTK->logv("arwGetMarkerPatternImage(): Marker with UID %d has no pattern with ID %d.", markerUID, patternID);
        return false;
    }
...
EXPORT_API bool arwGetMarkerOptionBool(int markerUID, int option)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwGetMarkerOptionBool(): Couldn't locate marker with UID %d.", markerUID);
        return false;
    }
    ...
    case ARW_MARKER_OPTION_SQUARE_USE_CONT_POSE_ESTIMATION:
        if (marker->type == ARMarker::SINGLE)
            return (((ARMarkerSquare*)marker)->useContPoseEstimation);

        break;

    default:
        gARTK->logv("arwGetMarkerOptionBool(): Unrecognised option %d.", option);
        break;
    }
...
EXPORT_API void arwSetMarkerOptionBool(int markerUID, int option, bool value)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwSetMarkerOptionBool(): Couldn't locate marker with UID %d.", markerUID);
        return;
    }
    ...
    case ARW_MARKER_OPTION_SQUARE_USE_CONT_POSE_ESTIMATION:
        if (marker->type == ARMarker::SINGLE)
            ((ARMarkerSquare*)marker)->useContPoseEstimation = value;

        break;

    default:
        gARTK->logv("arwSetMarkerOptionBool(): Unrecognised option %d.", option);
        break;
    }
}
...
EXPORT_API int arwGetMarkerOptionInt(int markerUID, int option)
{
    ...
    {
    default:
        gARTK->logv("arwGetMarkerOptionInt(): Unrecognised option %d.", option);
        break;
    }
...
EXPORT_API void arwSetMarkerOptionInt(int markerUID, int option, int value)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwSetMarkerOptionInt(): Couldn't locate marker with UID %d.", markerUID);
        return;
    }
    ...
    {
    default:
        gARTK->logv("arwSetMarkerOptionInt(): Unrecognised option %d.", option);
        break;
    }
}
...
EXPORT_API float arwGetMarkerOptionFloat(int markerUID, int option)
{
    ...
    case ARW_MARKER_OPTION_SQUARE_CONFIDENCE:
        if (marker->type == ARMarker::SINGLE)
            return ((float)((ARMarkerSquare*)marker)->getConfidence());
        else
            return (NAN);
    ...
    case ARW_MARKER_OPTION_SQUARE_CONFIDENCE_CUTOFF:
        if (marker->type == ARMarker::SINGLE)
            return ((float)((ARMarkerSquare*)marker)->getConfidenceCutoff());
        else
            return (NAN);
    ...
    case ARW_MARKER_OPTION_NFT_SCALE:
        ...
        if (marker->type == ARMarker::NFT)
            return ((float)((ARMarkerNFT*)marker)->getNFTScale());
        else
            return (NAN);
    ...
    default:
        gARTK->logv("arwGetMarkerOptionFloat(): Unrecognised option %d.", option);
        break;
    }
...
EXPORT_API void arwSetMarkerOptionFloat(int markerUID, int option, float value)
{
    ...
    if (!(marker = gARTK->findMarker(markerUID)))
    {
        gARTK->logv("arwSetMarkerOptionFloat(): Couldn't locate marker with UID %d.", markerUID);
        return;
    }
    ...
    case ARW_MARKER_OPTION_SQUARE_CONFIDENCE_CUTOFF:
        if (marker->type == ARMarker::SINGLE)
            ((ARMarkerSquare*)marker)->setConfidenceCutoff(value);
...
#if HAVE_NFT
        if (marker->type == ARMarker::NFT)
            ((ARMarkerNFT*)marker)->setNFTScale(value);
#endif
        break;

    default:
        gARTK->logv("arwSetMarkerOptionFloat(): Unrecognised option %d.", option);
        break;
    }
}
...
jfloatArray glArrayToJava(JNIEnv *env, ARdouble *arr, int len)
{
    ...
    if ((result = env->NewFloatArray(len)))
        env->SetFloatArrayRegion(result, 0, len, arr);
...
JNIEXPORT jboolean JNICALL JNIFUNCTION(arwChangeToResourcesDir(JNIEnv * env, jobject obj, jstring resourcesDirectoryPath))
{
    ...
    if (resourcesDirectoryPath != NULL)
    {
        ...
        ok = arwChangeToResourcesDir(NULL);
...
JNIEXPORT jboolean JNICALL JNIFUNCTION(arwGetVideoParams(JNIEnv * env, jobject obj, jintArray width, jintArray height, jintArray pixelSize, jobjectArray pixelFormatString))
{
    ...
    if (pixelSize)
        env->SetIntArrayRegion(pixelSize, 0, 1, &ps);

    if (pixelFormatString)
        env->SetObjectArrayElement(pixelFormatString, 0, env->NewStringUTF(pf));
...
JNIEXPORT jboolean JNICALL JNIFUNCTION(arwGetVideoParamsStereo(JNIEnv * env, jobject obj, jintArray widthL, jintArray heightL, jintArray pixelSizeL, jobjectArray pixelFormatStringL, jintArray widthR, jintArray heightR, jintArray pixelSizeR, jobjectArray pixelFormatStringR))
{
    ...
    if (!arwGetVideoParamsStereo(&wL, &hL, &psL, pfL, PIXEL_FORMAT_BUFFER_SIZE, &wR, &hR, &psR, pfR, PIXEL_FORMAT_BUFFER_SIZE))
        return false;

    if (widthL)
        env->SetIntArrayRegion(widthL, 0, 1, &wL);

    if (heightL)
        env->SetIntArrayRegion(heightL, 0, 1, &hL);

    if (pixelSizeL)
        env->SetIntArrayRegion(pixelSizeL, 0, 1, &psL);

    if (pixelFormatStringL)
        env->SetObjectArrayElement(pixelFormatStringL, 0, env->NewStringUTF(pfL));

    if (widthR)
        env->SetIntArrayRegion(widthR, 0, 1, &wR);

    if (heightR)
        env->SetIntArrayRegion(heightR, 0, 1, &hR);

    if (pixelSizeR)
        env->SetIntArrayRegion(pixelSizeR, 0, 1, &psR);

    if (pixelFormatStringR)
        env->SetObjectArrayElement(pixelFormatStringR, 0, env->NewStringUTF(pfR));
...
JNIEXPORT jfloatArray JNICALL JNIFUNCTION(arwGetProjectionMatrix(JNIEnv * env, jobject obj))
{
    ...
    if (arwGetProjectionMatrix(proj))
...
JNIEXPORT jboolean JNICALL JNIFUNCTION(arwGetProjectionMatrixStereo(JNIEnv * env, jobject obj, jfloatArray projL, jfloatArray projR))
{
    ...
    if (!arwGetProjectionMatrixStereo(pL, pR))
        return false;
...
JNIEXPORT jfloatArray JNICALL JNIFUNCTION(arwQueryMarkerTransformation(JNIEnv * env, jobject obj, jint markerUID))
{
    ...
    if (arwQueryMarkerTransformation(markerUID, trans))
        return glArrayToJava(env, trans, 16);
...
JNIEXPORT jboolean JNICALL JNIFUNCTION(arwQueryMarkerTransformationStereo(JNIEnv * env, jobject obj, jint markerUID, jfloatArray matrixL, jfloatArray matrixR))
{
    ...
    if (!arwQueryMarkerTransformationStereo(markerUID, mL, mR))
        return false;
