...
std::vector<ARMarker*> ARMarker::newFromConfigDataFile(const char *markersConfigDataFilePath, ARPattHandle *arPattHandle, int *patternDetectionMode_out)
{
    ...
    if (!markersConfigDataFilePath || markersConfigDataFilePath[0] == '\0')
...
#endif
                                   , &tempF) == 1)
                            tempObject->setFilterCutoffFrequency(tempF);
                    }
...
ARPattern* ARMarker::getPattern(int n)
{
    ...
    if (!patterns || n < 0 || n >= patternCount)
        return NULL;
...
void ARMarker::setFilterSampleRate(ARdouble rate)
{
    ...
    if (m_ftmi)
        arFilterTransMatSetParams(m_ftmi, m_filterSampleRate, m_filterCutoffFrequency);
}
