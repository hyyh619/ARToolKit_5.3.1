...
int kpmMergeRefDataSet(KpmRefDataSet **refDataSetPtr1, KpmRefDataSet **refDataSetPtr2)
{
    ...
    if ((*refDataSetPtr1)->refPoint != NULL)
        free((*refDataSetPtr1)->refPoint);
...
int kpmDeleteRefDataSet(KpmRefDataSet **refDataSetPtr)
{
    ...
    if (!*refDataSetPtr)
        return 0;                  // OK to call on already deleted handle.

    if ((*refDataSetPtr)->refPoint)
        free((*refDataSetPtr)->refPoint);
...
int kpmSaveRefDataSet(const char *filename, const char *ext, KpmRefDataSet  *refDataSet)
{
    ...
    if (fwrite(&(refDataSet->num), sizeof(int), 1, fp) != 1)
        goto bailBadWrite;

    for (i = 0; i < refDataSet->num; i++)
    {
        if (fwrite(&(refDataSet->refPoint[i].coord2D), sizeof(KpmCoord2D), 1, fp) != 1)
            goto bailBadWrite;

        if (fwrite(&(refDataSet->refPoint[i].coord3D), sizeof(KpmCoord2D), 1, fp) != 1)
            goto bailBadWrite;

#if BINARY_FEATURE
        if (fwrite(&(refDataSet->refPoint[i].featureVec), sizeof(FreakFeature), 1, fp) != 1)
            goto bailBadWrite;

#else
        if (fwrite(&(refDataSet->refPoint[i].featureVec), sizeof(SurfFeature), 1, fp) != 1)
            goto bailBadWrite;
#endif
        if (fwrite(&(refDataSet->refPoint[i].pageNo),     sizeof(int), 1, fp) != 1)
            goto bailBadWrite;

        if (fwrite(&(refDataSet->refPoint[i].refImageNo), sizeof(int), 1, fp) != 1)
            goto bailBadWrite;
    }

    if (fwrite(&(refDataSet->pageNum), sizeof(int), 1, fp) != 1)
        goto bailBadWrite;

    for (i = 0; i < refDataSet->pageNum; i++)
    {
        if (fwrite(&(refDataSet->pageInfo[i].pageNo),   sizeof(int), 1, fp) != 1)
            goto bailBadWrite;

        if (fwrite(&(refDataSet->pageInfo[i].imageNum), sizeof(int), 1, fp) != 1)
            goto bailBadWrite;

        j = refDataSet->pageInfo[i].imageNum;
        if (fwrite(refDataSet->pageInfo[i].imageInfo,  sizeof(KpmImageInfo), j, fp) != j)
            goto bailBadWrite;
    }
...
int kpmLoadRefDataSet(const char *filename, const char *ext, KpmRefDataSet **refDataSetPtr)
{
    ...
    if (fread(&(refDataSet->num), sizeof(int), 1, fp) != 1)
        goto bailBadRead;
    ...
    for (i = 0; i < refDataSet->num; i++)
    {
        if (fread(&(refDataSet->refPoint[i].coord2D), sizeof(KpmCoord2D), 1, fp) != 1)
            goto bailBadRead;

        if (fread(&(refDataSet->refPoint[i].coord3D), sizeof(KpmCoord2D), 1, fp) != 1)
            goto bailBadRead;

#if BINARY_FEATURE
        if (fread(&(refDataSet->refPoint[i].featureVec), sizeof(FreakFeature), 1, fp) != 1)
            goto bailBadRead;
        ...
        if (fread(&(refDataSet->refPoint[i].pageNo),     sizeof(int), 1, fp) != 1)
            goto bailBadRead;

        if (fread(&(refDataSet->refPoint[i].refImageNo), sizeof(int), 1, fp) != 1)
            goto bailBadRead;
    }

    if (fread(&(refDataSet->pageNum), sizeof(int), 1, fp) != 1)
        goto bailBadRead;
    ...
    for (i = 0; i < refDataSet->pageNum; i++)
    {
        if (fread(&(refDataSet->pageInfo[i].pageNo),   sizeof(int), 1, fp) != 1)
            goto bailBadRead;

        if (fread(&(refDataSet->pageInfo[i].imageNum), sizeof(int), 1, fp) != 1)
            goto bailBadRead;

        j = refDataSet->pageInfo[i].imageNum;
        arMalloc(refDataSet->pageInfo[i].imageInfo, KpmImageInfo, j);
        if (fread(refDataSet->pageInfo[i].imageInfo,  sizeof(KpmImageInfo), j, fp) != j)
            goto bailBadRead;
    }
...
bailBadRead:
    ...
    if (refDataSet->pageInfo)
        free(refDataSet->pageInfo);

    if (refDataSet->refPoint)
        free(refDataSet->refPoint);
...
int kpmLoadRefDataSetOld(const char *filename, const char *ext, KpmRefDataSet **refDataSetPtr)
{
    ...
    if (fread(dummy, sizeof(uint32_t), 2, fp) != 2)
        goto bailBadRead;                                              // Skip old (int)refDataSet->surfThresh and (int)refDataSet->coord3DFlag.
    ...
    for (i = 0; i < refDataSet->num; i++)
    {
        ...
        //    goto bailBadRead;
        // }
        if (fread(&(refDataSet->refPoint[i].featureVec.v[0]), sizeof(float), SURF_SUB_DIMENSION, fp) != SURF_SUB_DIMENSION)
            goto bailBadRead;
