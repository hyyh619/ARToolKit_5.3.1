...
ARVideoSourceInfoListT* ar2VideoCreateSourceInfoListWinDS(const char *config)
{
    ...
    {
        if ((ar2VideoWinDSDeviceInfo = ar2VideoWinDSInit2()) == NULL)
            return NULL;
    }
    ...
    for (i = 0; i < ar2VideoWinDSDeviceInfo->devNum; i++)
    {
        ...
        if (ar2VideoWinDSDeviceInfo->devStatus[i] == 1)
            list->info[i].flags |= AR_VIDEO_SOURCE_INFO_FLAG_IN_USE;
    }

// bail:
    ar2VideoWinDSDeviceInfoRefCount--;
    if (ar2VideoWinDSDeviceInfoRefCount == 0)
        ar2VideoWinDSFinal2(&ar2VideoWinDSDeviceInfo);
...
AR2VideoParamWinDST* ar2VideoOpenWinDS(const char *config)
{
    ...
    if (devNum == -1)
    {
        ...
        for (i = 0; i < ar2VideoWinDSDeviceInfo->devNum; i++)
        {
            if (ar2VideoWinDSDeviceInfo->devStatus[i] == 0)
                break;
        }
    ...
    if (showDialog == 1)
        ar2VideoWinDSDisplayPinProperties(pSrcOut);
...
STDMETHODIMP ARSampleGrabberCB::SampleCB(double Time, IMediaSample *pSample)
{
    ...
    if (buffer.status == AR2VIDEO_WINDS_STATUS_IDLE)
        return S_OK;
