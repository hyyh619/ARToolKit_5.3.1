    ...
    if (theMessage != WM_COMMAND)
    {
        ...
        if (movie)
        {
            if (movie->MC)
            {
                ...
                if (!IsIconic(theWnd))
                    MCIsPlayerEvent(movie->MC, &myMacEvent);
            }
        }
    }
...
AR_E_t arVideoQuickTimeMoviePlay(char *URL, char *conf, AR_VIDEO_QUICKTIME_MOVIE_t *movie_p)
{
    ...
    if (!gInited)
    {
        if ((err_e = arVideoQuickTimeMovieInit()) != AR_E_NONE)
            return (err_e);
    }
    ...
    if (conf)
    {
        ...
        for (;;)
        {
            ...
            {
                sscanf(a, "%s", line);
                if (strlen(line) <= 7 || sscanf(&line[7], "%d", &width) == 0)
                    err_i = 1;
            }
            else if (strncmp(a, "-height=", 8) == 0)
            {
                sscanf(a, "%s", line);
                if (strlen(line) <= 8 || sscanf(&line[8], "%d", &height) == 0)
                    err_i = 1;
            }
...
#ifdef AR_BIG_ENDIAN
                    if (strlen(line) == 17)
                        err_i = (sscanf(&line[13], "%4c", (char*)&pixFormat) < 1);

#else
                    if (strlen(line) == 17)
                        err_i = (sscanf(&line[13], "%c%c%c%c", &(((char*)&pixFormat)[3]), &(((char*)&pixFormat)[2]), &(((char*)&pixFormat)[1]), &(((char*)&pixFormat)[0])) < 1);
#endif
                    else
                        err_i = (sscanf(&line[13], "%li", (long*)&pixFormat) < 1);                       // Integer.
                }
            }
            ...
            {
                sscanf(a, "%s", line);
                if (strlen(line) <= 6 || sscanf(&line[6], "%f", &rate) == 0)
                    err_i = 1;
            }
            else
            {
    ...
    {
        movieIndex++;
        if (movieIndex == AR_VIDEO_QUICKTIME_MOVIE_COUNT_MAX)
            movieIndex = 0;                                                           // Loop around.

        if (movieIndex == gMovieIndex)
            return (AR_E_OVERFLOW);                                    // No free slots = too many movies loaded.
    }
    ...
    if (!(movie = (AR_VIDEO_QUICKTIME_MOVIE_t)calloc(1, sizeof(struct _AR_VIDEO_QUICKTIME_MOVIE_t))))
        return (AR_E_OUT_OF_MEMORY);
...
# ifdef _WIN32
    ...
    else
    {
        ...
        if (!width)
            width = movie->sourceWidth;

        if (!height)
            height = movie->sourceHeight;
    ...
    if (!showController)
        flags = mcNotVisible | mcTopLeftMovie;
    ...
    if (pause || playEveryFrame)
        rateFixed = IntToFixed(0);
    else if (rate != 0.0f)
        rateFixed = FloatToFixed(rate);
    else
        rateFixed = GetMoviePreferredRate(movie->movie);
...
AR_E_t arVideoQuickTimeMovieGetPlayLength(AR_VIDEO_QUICKTIME_MOVIE_t movie, double *lengthInSeconds)
{
    ...
    if (!movie->movieTimeValueDuration || !movie->movieTimeScale)
        return (AR_E_LENGTH_UNAVAILABLE);
...
AR_E_t arVideoQuickTimeMovieSetPlayPosition(AR_VIDEO_QUICKTIME_MOVIE_t movie, double positionInSeconds)
{
    ...
    if (tvToSet < 0 || tvToSet > movie->movieTimeValueDuration)
        return (AR_E_OVERFLOW);
...
unsigned char* arVideoQuickTimeMovieGetFrame(AR_VIDEO_QUICKTIME_MOVIE_t movie, double *time)
{
    ...
    if (movie->status & AR_VIDEO_QUICKTIME_MOVIE_STATUS_BIT_READY)
    {
        ...
        if (time)
            *time = (double)movie->movieTime / (double)movie->movieTimeScale;
    }
...
AR_E_t arVideoQuickTimeMovieGetFrameSize(AR_VIDEO_QUICKTIME_MOVIE_t movie, unsigned int *width, unsigned int *height, unsigned int *pixelSize, unsigned int *pixFormat)
{
    ...
    if (width)
        *width = movie->destWidth;

    if (height)
        *height = movie->destHeight;

    if (pixelSize)
        *pixelSize = movie->rowBytes / movie->destWidth;

    if (pixFormat)
        *pixFormat = movie->pixFormatEnum;
