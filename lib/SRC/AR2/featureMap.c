...
int ar2SaveFeatureMap(char *filename, char *ext, AR2FeatureMapT *featureMap)
{
    ...
    if (fwrite(&(featureMap->xsize), sizeof(featureMap->xsize), 1, fp) != 1)
        goto bailBadWrite;

    if (fwrite(&(featureMap->ysize), sizeof(featureMap->ysize), 1, fp) != 1)
        goto bailBadWrite;

    if (fwrite(featureMap->map, sizeof(float), (featureMap->xsize) * (featureMap->ysize), fp) != (featureMap->xsize) * (featureMap->ysize))
        goto bailBadWrite;
                                 ...
                                 float max_sim_thresh, float sd_thresh)
{
    ...
    {
        j += hist[i];
        if ((float )j / (float )(xsize * ysize) >= 0.02f)
            break;
                                   ...
                                   float max_sim_thresh, float min_sim_thresh, float sd_thresh, int *num)
{
    ...
    if (image->xsize != featureMap->xsize || image->ysize != featureMap->ysize)
        return NULL;
...
#if AR2_CAPABLE_ADAPTIVE_TEMPLATE
                if (get_similarity(image->imgBWBlur[1], xsize, ysize, template, vlen, ts1, ts2, cx + i, cy + j, &sim) < 0)
                    continue;

#else
                if (get_similarity(image->imgBW, xsize, ysize, template, vlen, ts1, ts2, cx + i, cy + j, &sim) < 0)
                    continue;
#endif
