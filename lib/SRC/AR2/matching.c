                       ...
                       int search[3][2], int *bx, int *by, float *val)
{
    ...
    for (ii = 0; ii < 3; ii++)
    {
        ...
        {
            if (ret)
                return -1;       // If we haven't got at least one starting point for a search, bail out.
            else
                break;
        }
        ...
        for (j = py - ry; j <= py + ry; j += SKIP_INTERVAL + 1)
        {
            ...
            for (i = px - rx; i <= px + rx; i += SKIP_INTERVAL + 1)
            {
                ...
                if (mfImage[j * xsize + i])
                    continue;                        // Skip pixels already matched.
                                     ...
                                     AR2TemplateT *mtemp, int sx, int sy, int *val)
{
    ...
    if (vlen == 0)
        ...
        *val = sum3 * 100 / mtemp->vlen * 100 / (int)sqrtf((float)vlen);
