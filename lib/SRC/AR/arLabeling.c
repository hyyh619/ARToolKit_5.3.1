               ...
               ARLabelInfo *labelInfo, ARUint8 *image_thresh)
{
    ...
    if (labelingMode == AR_LABELING_BLACK_REGION)
    {
        ...
        if (image_thresh)
            return arLabelingSubDBZ(image, xsize, ysize, image_thresh, labelInfo);
#endif
...
#if !AR_DISABLE_THRESH_MODE_AUTO_ADAPTIVE
        if (image_thresh)
            return arLabelingSubDWZ(image, xsize, ysize, image_thresh, labelInfo);
#endif
        if (imageProcMode == AR_IMAGE_PROC_FRAME_IMAGE)
        {
            if (pixFormat == AR_PIXEL_FORMAT_RGB || pixFormat == AR_PIXEL_FORMAT_BGR)
...
#if !AR_DISABLE_THRESH_MODE_AUTO_ADAPTIVE
        if (image_thresh)
            return arLabelingSubEBZ(image, xsize, ysize, image_thresh, labelInfo);
#endif
        if (imageProcMode == AR_IMAGE_PROC_FRAME_IMAGE)
        {
            if (pixFormat == AR_PIXEL_FORMAT_RGB || pixFormat == AR_PIXEL_FORMAT_BGR)
...
#if !AR_DISABLE_THRESH_MODE_AUTO_ADAPTIVE
        if (image_thresh)
            return arLabelingSubEWZ(image, xsize, ysize, image_thresh, labelInfo);
#endif
        if (imageProcMode == AR_IMAGE_PROC_FRAME_IMAGE)
        {
            if (pixFormat == AR_PIXEL_FORMAT_RGB || pixFormat == AR_PIXEL_FORMAT_BGR)
