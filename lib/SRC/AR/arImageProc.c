...
#  endif
        if (ipi->fastPath)
            ARLOGd("arImageProc will use ARM NEON acceleration.\n");
#endif
    }
...
int arImageProcLumaHist(ARImageProcInfo *ipi, const ARUint8* __restrict dataPtr)
{
    ...
    for (p = ipi->image; p < ipi->image + ipi->imageX * ipi->imageY; p++)
        ipi->histBins[*p]++;
...
int arImageProcLumaHistAndBoxFilterWithBias(ARImageProcInfo *ipi, const ARUint8* __restrict dataPtr, const int boxSize, const int bias)
{
    ...
    if (!ipi->tempBuffer)
    {
        ...
        if ((err = vImageBoxConvolve_Planar8(&src, &dest, NULL, 0, 0, boxSize, boxSize, '\0', kvImageTruncateKernel | kvImageGetTempBufferSize)) < 0)
            return (-1);
