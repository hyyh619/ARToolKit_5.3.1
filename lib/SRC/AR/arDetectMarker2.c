                    ...
                    ARMarkerInfo2 *markerInfo2, int *marker2_num)
{
    ...
    *marker2_num = 0;
    ...
    {
        if (labelInfo->area[i] < areaMin || labelInfo->area[i] > areaMax)
            continue;

        if (labelInfo->clip[i][0] == 1 || labelInfo->clip[i][1] == xsize - 2)
            continue;

        if (labelInfo->clip[i][2] == 1 || labelInfo->clip[i][3] == ysize - 2)
            continue;
                 ...
                 int clip[4], ARMarkerInfo2 *marker_info2)
{
    ...
    for (;;)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (p1[ydir[dir] * xsize + xdir[dir]] > 0)
                break;
        ...
        if (marker_info2->x_coord[marker_info2->coord_num] == sx
            && marker_info2->y_coord[marker_info2->coord_num] == sy)
            break;
