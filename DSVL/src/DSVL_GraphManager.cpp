...
{
    if (device > INVALID_INPUT_FILTER)
        return(NULL);
    else
        return(VIDEO_INPUT_DEVICE_names[device]);
}
...
VIDEO_INPUT_DEVICE StringVideoInputDevice(char *device_str)
{
    for (unsigned int i = 0; i < INVALID_INPUT_FILTER; i++)
        if (strcmp(device_str, VIDEO_INPUT_DEVICE_names[i]) == 0)
            return((VIDEO_INPUT_DEVICE)i);
                                                 ...
                                                 PIXELFORMAT *pixel_format)
{
    ...
    if (frame_width != NULL)
        *frame_width = mf.biWidth;

    if (frame_height != NULL)
        *frame_height = mf.biHeight;

    if (frames_per_second != NULL)
        *frames_per_second = mf.frameRate;

    if (pixel_format != NULL)
        *pixel_format = mf.pixel_format;
                                              ...
                                              unsigned int _allocatorBuffersPerClient)
{
    ...
    if (FAILED(hr = getPin(grabberFilter, PINDIR_INPUT, 1, sgPin)))
        return(hr);
...
HRESULT DSVL_GraphManager::GetCameraParameterN(CP_INTERFACE interface_type, long Property, double *dValue)
{
    ...
    if (FAILED(hr = GetCameraParameter(interface_type, Property, &oldValue, &oldFlags)))
        return(hr);
...
HRESULT DSVL_GraphManager::Pause()
{
    ...
    if (FAILED(hr = mediaControl->Pause()))
        return(hr);
    else
        return(S_OK);
}
...
DWORD DSVL_GraphManager::WaitForNextSample(long dwMilliseconds)
{
    ...
    do
    {
        ...
        if (dw == WAIT_OBJECT_0 || dw == WAIT_TIMEOUT)
        {
            ...
            while (PeekMessage(&uMsg, NULL, 0, 0, PM_NOREMOVE))
            {
...
HRESULT DSVL_GraphManager::ReleaseGraph()
{
    ...
    if (!m_bGraphIsInitialized)
        return(E_FAIL);                            // call BuildGraph() first!
...
HRESULT DSVL_GraphManager::BuildGraphFromXMLFile(char *xml_filename)
{
    ...
    if (!xmlDoc.LoadFile())
    {
        ...
        {
            if (!xmlDoc.LoadFile(_bstr_t(path)))
                return(E_INVALIDARG);
        }
    }
...
{
    if (m_bGraphIsInitialized)
        return(E_FAIL);                           // call ReleaseGraph() first!
    ...
    if (FAILED(pSampleGrabber->QueryInterface(IID_ISampleGrabber, (void**)&sampleGrabber)))
        return(hr);
    ...
    if (mf.inputDevice == WDM_VIDEO_CAPTURE_FILTER)
    {
        ...
        if (FAILED(hr = getPin(pVideoSource, PINDIR_OUTPUT, 1, capturePin)))
            return(hr);
        ...
        if (pinSupportsDV)
        {
            ...
            if (FAILED(hr = CoCreateInstance(CLSID_DVVideoCodec, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&(pVideoDecoder))))
                return(hr);
            ...
            {
                if (FAILED(pVideoDecoder->QueryInterface(IID_IIPDVDec, (void**)&pDVDec)))
                    return(hr);

                int dvRes;
                if (FAILED(hr = pDVDec->get_IPDisplay(&dvRes)))
                    return(hr);                                                            // get default resolution

                if ((mf.biWidth == 720) && ((mf.biHeight == 480) || (mf.biHeight == 576)))
                    dvRes = DVRESOLUTION_FULL;
                else if ((mf.biWidth == 360) && ((mf.biHeight == 240) || (mf.biHeight == 288)))
                    dvRes = DVRESOLUTION_HALF;
                else if ((mf.biWidth == 180) && ((mf.biHeight == 120) || (mf.biHeight == 144)))
                    dvRes = DVRESOLUTION_QUARTER;
                else if ((mf.biWidth == 88) && ((mf.biHeight == 60) || (mf.biHeight == 72)))
                    dvRes = DVRESOLUTION_DC;

                if (FAILED(hr = pDVDec->put_IPDisplay(dvRes)))
                    return(hr);
            }
        ...
        else         // !pinSupportsDV
        {
            if (mf.inputFlags & WDM_MATCH_FORMAT)
            {
                ...
                if (FAILED(hr = MatchMediaTypes(capturePin, &mf, &mt)))
                {
                    ...
                    pStreamConfig->SetFormat(&mt);
            }

            if ((mf.inputFlags & WDM_SHOW_FORMAT_DIALOG) && !pinSupportsDV)
                if (FAILED(hr = DisplayPinProperties(capturePin)))
                    return(hr);
        }         // !pinSupportsDV
        ...
        if (mf.inputFlags & WDM_SHOW_CONTROL_DIALOG)
            if (FAILED(hr = DisplayFilterProperties(sourceFilter)))
                return(hr);
    ...
    else if (mf.inputDevice == ASYNC_FILE_INPUT_FILTER)
    {
        ...
        if (FAILED(hr = CoCreateInstance(CLSID_AsyncReader, NULL, CLSCTX_INPROC_SERVER,
                                         IID_IBaseFilter, (void**)&(pVideoSource))))
            return(hr);

        CComPtr<IFileSourceFilter> pFileSource = NULL;
        if (FAILED(pVideoSource->QueryInterface(IID_IFileSourceFilter, (void**)&pFileSource)))
            return(hr);
        ...
        if (FAILED(hr = graphBuilder->AddFilter(pStreamSplitter, L"Stream Splitter")))
            return(hr);
        ...
        if (FAILED(hr = getPin(pStreamSplitter, PINDIR_OUTPUT, 1, pStreamPin00)))
            return(hr);
    ...
    CComPtr<IPin> sgPin      = NULL;
    if (FAILED(hr = getPin(pSampleGrabber, PINDIR_INPUT, 1, sgPin)))
        return(hr);
    ...
    ALLOCATOR_PROPERTIES actualProperties;
    pAllocator->GetProperties(&requestedProperties);
    if (requestedProperties.cBuffers != _num_alloc)
        requestedProperties.cBuffers = _num_alloc;
