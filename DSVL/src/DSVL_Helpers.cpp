                       ...
                       IBaseFilter *in_filter, int in_pin_nr)
{
    ...
    if (FAILED(hr = getPin(filter_out, PINDIR_OUTPUT, out_pin_nr, OutPin)))
        return(hr);
    ...
    if (FAILED(hr = OutPin->Connect(InPin, NULL)))
        return(E_FAIL);
    else
        return(S_OK);
}
                           ...
                           int in_pin_nr, IGraphBuilder *pGraphBuilder)
{
    ...
    if (FAILED(hr = pGraphBuilder->Connect(OutPin, InPin)))
        return(E_FAIL);
    else
        return(S_OK);
}
                ...
                if (SUCCEEDED(name_hr) && wcsstr(varName.bstrVal, filterNameSubstring) != NULL)
                {
                    ...
                    if (ieee1394id_str != NULL)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            if (strcmp(ieee1394id_str, temp_str) == 0)
                                matching_id = true;
...
HRESULT AMErrorMessage(HRESULT hr, char *error_title)
{
    ...
    if (cRet > 0)
        MessageBox(NULL, _T(buffer), error_title, MB_OK);
    else
        return(E_FAIL);
